...
async function loadStudents() {
    try {
        ...
        tbody.innerHTML = students.length ? students.map(s => `
            <tr class="student-row-item" data-name="${s.name.toLowerCase()}" data-roll="${s.studentId.toLowerCase()}">
                <td><strong>${s.id}</strong></td>
                <td>${s.name}</td>
                <td><span class="grade-badge A">${s.studentId}</span></td>
                <td><span class="grade-badge ${s.branch === 'Computer Science' ? 'O' : s.branch === 'Information Technology' ? 'B-plus' : 'A-plus'}" style="border: none; padding: 0.15rem 0.5rem; font-size: 0.75rem;">${s.branch || 'Computer Science'}</span></td>
                <td style="text-align: right;">
                    <div class="table-actions" style="justify-content: flex-end;">
                         <button class="btn btn-secondary btn-sm" onclick="editStudent(${s.id}, '${s.name}', '${s.studentId}', '${s.branch}')">Edit</button>
        ...
        `).join('') : '<tr><td colspan="5" class="empty-state">No student accounts registered in the database.</td></tr>';
...
async function addFaculty() {
    ...
    if (!name || !username || !email || !department) {
        showToast('Complete all faculty fields.', 'error');
        return;
    }
...
async function addDepartment() {
    ...
    if (!name || !code) {
        showToast('Complete all department fields.', 'error');
        return;
    }
