...
function setupEventListeners() {
    ...
    ['students', 'courses', 'enrollments', 'faculty', 'departments'].forEach(type => {
        ...
        if (input) {
            const debouncedFilter = UI.debounce(() => filterTable(type), 250);
...
function setValidationState(inputEl, isValid, message = '') {
    ...
    if (isValid) {
        ...
        let feedback = inputEl.parentNode.querySelector('.validation-feedback');
        if (!feedback) {
...
function clearValidationStates(tabContentId) {
    const tab = document.getElementById(tabContentId) || document;
    tab.querySelectorAll('.is-valid, .is-invalid').forEach(el => {
        el.classList.remove('is-valid', 'is-invalid');
    });
    tab.querySelectorAll('.validation-feedback').forEach(el => {
        el.remove();
    });
}
...
function setupRealTimeValidation() {
    ...
    if (sName) sName.addEventListener('input', () => {
        const val = sName.value.trim();
        if (!val) setValidationState(sName, false, 'Student name is required');
        else if (val.length < 2) setValidationState(sName, false, 'Name must be at least 2 characters');
        else setValidationState(sName, true);
    });

    const sId = document.getElementById('student-id');
    if (sId) sId.addEventListener('input', () => {
        const val = sId.value.trim();
        const idRegex = /^[A-Za-z0-9]{3,15}$/;
        if (!val) setValidationState(sId, false, 'Student ID is required');
        else if (!idRegex.test(val)) setValidationState(sId, false, 'ID must be 3-15 alphanumeric characters');
        else setValidationState(sId, true);
    });

    const sDob = document.getElementById('student-dob');
    if (sDob) sDob.addEventListener('change', () => {
        const val = sDob.value;
        if (!val) setValidationState(sDob, false, 'Date of Birth is required');
        else {
            const date = new Date(val);
            if (date >= new Date()) setValidationState(sDob, false, 'Date of Birth must be in the past');
            else setValidationState(sDob, true);
        }
    });

    // Faculty fields
    const fName = document.getElementById('faculty-name');
    if (fName) fName.addEventListener('input', () => {
        const val = fName.value.trim();
        if (!val) setValidationState(fName, false, 'Faculty name is required');
        else setValidationState(fName, true);
    });

    const fUsername = document.getElementById('faculty-username');
    if (fUsername) fUsername.addEventListener('input', () => {
        const val = fUsername.value.trim();
        if (!val) setValidationState(fUsername, false, 'Username is required');
        else if (val.length < 3) setValidationState(fUsername, false, 'Username must be at least 3 characters');
        else setValidationState(fUsername, true);
    });

    const fEmail = document.getElementById('faculty-email');
    if (fEmail) fEmail.addEventListener('input', () => {
        const val = fEmail.value.trim();
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!val) setValidationState(fEmail, false, 'Email address is required');
        else if (!emailRegex.test(val)) setValidationState(fEmail, false, 'Provide a valid email format');
        else setValidationState(fEmail, true);
    });

    // Department fields
    const dName = document.getElementById('department-name');
    if (dName) dName.addEventListener('input', () => {
        const val = dName.value.trim();
        if (!val) setValidationState(dName, false, 'Department name is required');
        else setValidationState(dName, true);
    });

    const dCode = document.getElementById('department-code');
    if (dCode) dCode.addEventListener('input', () => {
        const val = dCode.value.trim();
        const codeRegex = /^[A-Za-z0-9]{2,5}$/;
        if (!val) setValidationState(dCode, false, 'Department code is required');
        else if (!codeRegex.test(val)) setValidationState(dCode, false, 'Code must be 2-5 alphanumeric characters');
        else setValidationState(dCode, true);
    });

    // Semester fields
    const semNum = document.getElementById('semester-number');
    if (semNum) semNum.addEventListener('input', () => {
        const val = parseInt(semNum.value.trim());
        if (isNaN(val) || val < 1 || val > 10) setValidationState(semNum, false, 'Semester sequence must be between 1 and 10');
        else setValidationState(semNum, true);
    });

    // Course fields
    const cCode = document.getElementById('course-code');
    if (cCode) cCode.addEventListener('input', () => {
        const val = cCode.value.trim();
        const codeRegex = /^[A-Za-z0-9-]{3,10}$/;
        if (!val) setValidationState(cCode, false, 'Course Code is required');
        else if (!codeRegex.test(val)) setValidationState(cCode, false, 'Code must be 3-10 alphanumeric/hyphen characters');
        else setValidationState(cCode, true);
    });

    const cName = document.getElementById('course-name');
    if (cName) cName.addEventListener('input', () => {
        const val = cName.value.trim();
        if (!val) setValidationState(cName, false, 'Course Title is required');
        else if (val.length < 3) setValidationState(cName, false, 'Title must be at least 3 characters');
        else setValidationState(cName, true);
    });

    const cCredits = document.getElementById('course-credits');
    if (cCredits) cCredits.addEventListener('input', () => {
        const val = parseInt(cCredits.value.trim());
        if (isNaN(val) || val < 1 || val > 6) setValidationState(cCredits, false, 'Credits must be between 1 and 6');
        else setValidationState(cCredits, true);
    });
}
...
async function addStudent() {
    ...
    studentIdEl.dispatchEvent(new Event('input'));
    dobEl.dispatchEvent(new Event('change'));

    if (nameEl.classList.contains('is-invalid') || studentIdEl.classList.contains('is-invalid') || dobEl.classList.contains('is-invalid')) {
    ...
    try {
        ...
        nameEl.value = '';
        studentIdEl.value = '';
        dobEl.value = '2004-01-01';
        clearValidationStates('tab-students');
        await loadStudents();
...
async function addFaculty() {
    ...
    const usernameEl = document.getElementById('faculty-username');
    ...
    if (nameEl.classList.contains('is-invalid') || usernameEl.classList.contains('is-invalid') || emailEl.classList.contains('is-invalid')) {
        Toast.error('Please fix validation errors first.');
        return;
    }
    ...
    try {
        ...
        usernameEl.value = '';
        emailEl.value = '';
        clearValidationStates('tab-faculty');
        await loadFaculty();
...
async function addDepartment() {
    ...
    if (nameEl.classList.contains('is-invalid') || codeEl.classList.contains('is-invalid')) {
        Toast.error('Please fix validation errors first.');
        return;
    }
    ...
    try {
        ...
        nameEl.value = '';
        codeEl.value = '';
        clearValidationStates('tab-departments');
        await loadDepartments();
...
async function addSemester() {
    const numEl = document.getElementById('semester-number');
    numEl.dispatchEvent(new Event('input'));

    if (numEl.classList.contains('is-invalid')) {
        Toast.error('Please fix validation errors first.');
        return;
    }
    ...
    try {
        ...
        numEl.value = '';
        clearValidationStates('tab-semesters');
        await loadSemesters();
...
async function addCourse() {
    ...
    codeEl.dispatchEvent(new Event('input'));
    nameEl.dispatchEvent(new Event('input'));
    creditsEl.dispatchEvent(new Event('input'));

    if (codeEl.classList.contains('is-invalid') || nameEl.classList.contains('is-invalid') || creditsEl.classList.contains('is-invalid')) {
        Toast.error('Please fix validation errors first.');
        return;
    }
    ...
    try {
        ...
        nameEl.value = '';
        creditsEl.value = '';
        clearValidationStates('tab-courses');
        await loadCourses();
